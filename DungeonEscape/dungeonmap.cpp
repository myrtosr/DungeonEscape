...
void DungeonMap::clear()
{
    ...
    for (RoomView* rv : roomViews) {
        std::cout << "[DEBUG] Deleting roomView of room " << rv->getRoomNode()->getId() << std::endl;
        delete rv;
    }
    roomViews.clear();
    std::cout << "[DEBUG] Structure used for roomViews cleared!" << std::endl;

    // Delete passageViews
    for (PassageView* pv : passageViews) {
        std::cout << "[DEBUG] Deleting passageView of passage " << pv->getPassage()->getId() << std::endl;
        delete pv;
    }
    passageViews.clear();
    std::cout << "[DEBUG] Structure used for passageViews cleared!" << std::endl;
...
void DungeonMap::buildViews()
{
    ...
    roomViews.push_back(new RoomView(r1, { 5, 1 }, { 9, 5 }));
    roomViews[0]->addEntrance({ 7,5 });

    RoomNode* r2 = graph->getRoomById(2);
    roomViews.push_back(new RoomView(r2, { 4, 9 }, { 10, 13 }));
    roomViews[1]->addEntrance({ 7, 9 });
    roomViews[1]->addEntrance({ 4, 11 });
    roomViews[1]->addEntrance({ 10, 11 });

    RoomNode* r3 = graph->getRoomById(3);
    roomViews.push_back(new RoomView(r3, { 1, 14 }, { 3, 18 }));
    // no entrances, only door

    RoomNode* r4 = graph->getRoomById(4);
    roomViews.push_back(new RoomView(r4, { 14, 9 }, { 18, 13 }));
    roomViews[3]->addEntrance({ 16, 9 });
    roomViews[3]->addEntrance({ 16, 13 });

    RoomNode* r5 = graph->getRoomById(5);
    roomViews.push_back(new RoomView(r5, { 15, 1 }, { 17, 5 }));
    // no entrances, only door

    RoomNode* r6 = graph->getRoomById(6);
    roomViews.push_back(new RoomView(r6, { 4, 18 }, { 10, 26 }));
    // no entrances, only door

    RoomNode* r7 = graph->getRoomById(7);
    roomViews.push_back(new RoomView(r7, { 13, 18 }, { 18, 26 }));
    roomViews[6]->addEntrance({ 16, 26 });

    RoomNode* r8 = graph->getRoomById(8);
    roomViews.push_back(new RoomView(r8, { 13, 30 }, { 18, 33 }));
    roomViews[7]->addEntrance({ 13, 32 });


    // Passage Initialization
    Passage* p1 = graph->getPassageById(1);
    passageViews.push_back(new PassageView(p1, { 7, 6 }, { 7, 8 }));

    Passage* p2 = graph->getPassageById(2);
    passageViews.push_back(new PassageView(p2, { 3, 11 }, { 2, 13 }, {2, 11})); // corner passage

    Passage* p3 = graph->getPassageById(3);
    passageViews.push_back(new PassageView(p3, { 11, 11 }, { 13, 11 }));

    Passage* p4 = graph->getPassageById(4);
    passageViews.push_back(new PassageView(p4, { 16, 6 }, { 16, 8 }));

    Passage* p5 = graph->getPassageById(5);
    passageViews.push_back(new PassageView(p5, { 16, 14 }, { 16, 17 }));

    Passage* p6 = graph->getPassageById(6);
    passageViews.push_back(new PassageView(p6, { 16, 14 }, { 7, 17 }, { 16, 17 })); // corner passage

    Passage* p7 = graph->getPassageById(7);
    passageViews.push_back(new PassageView(p7, { 16, 17 }, { 7, 17 }));

    Passage* p8 = graph->getPassageById(8);
    passageViews.push_back(new PassageView(p8, { 7, 14 }, { 7, 17 }));

    Passage* p9 = graph->getPassageById(9);
    passageViews.push_back(new PassageView(p9, { 7, 14 }, { 16, 17 },{ 7, 17 })); // corner passage

    Passage* p10 = graph->getPassageById(10);
    passageViews.push_back(new PassageView(p10, { 16, 27 }, { 16, 29 }));
...
std::vector<TileCoord> DungeonMap::findFullPath(TileCoord startTile, TileCoord targetTile)
{
    ...
    // Call simple BFS to calculate shortest path from startTile -> targetTile
    ...
    // The path is given in a vector of roomIDs in the order we must traverse the rooms
    ...
    for (size_t i = 0; i < roomPath.size() - 1; ++i) { // For each consecutive pair of rooms
        ...
        if (passage->isCornered()) {
            waypoints.push_back(passage->getCorner()); // Γ-shaped passage -> extra waypoint for turn
        }
    ...
    for (size_t i = 0; i < waypoints.size() - 1; ++i) {
        ...
        std::vector<TileCoord> segment = tileMap.findTilePath(from, to); // calling BFS to move between waypoints
...
{
    // Draw tiles on grid (rooms, passages, doors)
    tileMap.draw();
